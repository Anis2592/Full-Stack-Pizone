        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {employees.map((employee) => (
            <div key={employee._id} className="bg-gray-50 p-5 shadow-md rounded-lg">
              ...
              <div className="flex justify-between mt-3">
  <Link to={`/view-employee/${employee._id}`} className="bg-blue-500 text-white px-3 py-1 rounded">
    View
  </Link>
  <Link to={`/update-employee/${employee._id}`} className="bg-yellow-500 text-white px-3 py-1 rounded">
    Edit
  </Link>
  <button onClick={() => deleteEmployee(employee._id)} className="bg-red-500 text-white px-3 py-1 rounded">
    Delete
  </button>
</div>
